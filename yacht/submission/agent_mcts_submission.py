...
class Game:
    ...
    def _calculate_category_score(self, cat: int, dice: List[int]) -> int:
        ...
        if cat == 0: return sum(d for d in dice if d == 1) * 1000
        if cat == 1: return sum(d for d in dice if d == 2) * 1000
        if cat == 2: return sum(d for d in dice if d == 3) * 1000
        if cat == 3: return sum(d for d in dice if d == 4) * 1000
        if cat == 4: return sum(d for d in dice if d == 5) * 1000
        if cat == 5: return sum(d for d in dice if d == 6) * 1000
        if cat == 6: return sum(dice) * 1000
        if cat == 7:
        ...
        if cat == 8:
            ...
            for i in range(1, 7):
                cnt = dice.count(i)
                if cnt == 2 or cnt == 5: pair = True
                if cnt == 3 or cnt == 5: triple = True
    ...
    def _board_to_vector(self, board: dict) -> np.ndarray:
        ...
        if board['phase'] == 'bid': vec[idx] = 1.0
        idx += 1
        if board['phase'] == 'score': vec[idx] = 1.0
        idx += 1
...
class Net(nn.Module):
    ...
    def __init__(self, game: Game):
        ...
        input_dim = self.board_size[0] * self.board_size[1] * self.board_size[2]
        hidden_dim = 256
...
class NNetWrapper:
    ...
    def predict(self, board: Dict) -> Tuple[np.ndarray, float]:
        ...
        with torch.no_grad():
            ...
            board_tensor = torch.tensor(board_vec, dtype=torch.float32, device=self.device).unsqueeze(0)
...
def find_subset_index(dice_pool: List[int], selected: List[int], subsets: List[Tuple[int, ...]]) -> int:
    ...
    for idx, subset in enumerate(subsets):
        if max(subset) >= n or len(subset) != required:
            continue
...
def main() -> None:
    ...
    try:
        ...
        try:
            ...
            print(f"Warning: Could not load trained model from {model_path}", file=sys.stderr)
